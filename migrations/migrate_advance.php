...
{
    $result = fetch_once(0, 'main', 'SELECT MAX(batch) FROM migrations');
    return ++$result;
}
...
function migrate_advance_undo_done()
{
    ...
    for ($i = count($migrate_advance_done) - 1; $i >= 0; $i--) {
        ...
        query('main', 'DELETE FROM migrations WHERE name = $1', $name);
    }
...
function migrate_advance_make(string $name, int $batch): bool
{
    ...
    if ($result) {
        ...
        if (query('main', 'INSERT INTO migrations (name, batch) VALUES ($1, $2)',
            $name, $batch)) {
            return true;
        }
    }
...
function migrate_advance_process(string $name, int $batch): bool
{
    ...
    $done = fetch_once(0, 'main', $sql, $name);
    if (!$done) {
